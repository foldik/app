...
toyyyyMMddhhmm posix zone =
    let
        posixTime = Time.millisToPosix posix
        year = String.fromInt (Time.toYear zone posixTime)
        month = toMonthNumberSting (Time.toMonth zone posixTime)
        day = String.fromInt (Time.toDay zone posixTime)
        hour = String.fromInt (Time.toHour zone posixTime)
        minute = String.fromInt (Time.toMinute zone posixTime)
    in
